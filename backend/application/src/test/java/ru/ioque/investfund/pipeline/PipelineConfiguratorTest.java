...
public class PipelineConfiguratorTest extends BaseTest {
    ...
    void beforeEach() {
        ...
        pipelineManager().initializePipeline(getDatasourceId());
    ...
    @DisplayName("""
        ...
        """)
    void testCase1() {
        final Flux<IntradayData> intradayDataStream = intradayJournal().stream();
        final Flux<Signal> signalStream = signalJournal().stream();
        intradayDataStream.subscribe(signalProducer()::process);
        intradayDataStream.subscribe(riskManager()::process);
        signalStream.subscribe(positionManager()::process);
        signalStream.subscribe(telegramPublisher()::process);
